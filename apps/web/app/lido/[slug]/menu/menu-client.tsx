...
export default function MenuClient({ establishment, categories, items }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!result.success) {
      setOrderError(result.error ?? "Errore durante l'invio dell'ordine. Riprova.");
      setSubmitting(false);
      return;
    }
        ...
        <Card className="max-w-sm w-full">
          <CardContent className="p-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Il tuo ordine è stato ricevuto. Ti porteremo tutto direttamente
              all&apos;ombrellone{" "}
      ...
      <div className="flex min-h-screen items-center justify-center bg-background px-4">
        <Card className="max-w-sm w-full">
          <CardContent className="p-6 text-center">
            ...
            <p className="mt-4 font-medium">Menu non disponibile</p>
            <Button variant="outline" className="mt-4" asChild>
              <Link href={`/lido/${establishment.slug}`}>
                Torna alla pagina
              </Link>
            </Button>
          </CardContent>
